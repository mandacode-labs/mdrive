...
func TestRm_SuccessSingleFile(t *testing.T) {
	...
	// Batch unlink
	inodeSvc.On("UnlinkBatch", mock.Anything, "root-id", []string{"file"}).Return(nil)
...
func TestRm_SuccessMultipleFiles(t *testing.T) {
	...
	// Batch unlink
	inodeSvc.On("UnlinkBatch", mock.Anything, "root-id", []string{"file1", "file2"}).Return(nil)
...
func TestRm_MixedSuccessFailure(t *testing.T) {
	...
	inodeSvc.On("UnlinkBatch", mock.Anything, "root-id", []string{"file1"}).Return(nil)
...
func TestRm_NonEmptyDirectory(t *testing.T) {
	...
	inodeSvc.On("ReadDir", mock.Anything, "dir-id").Return([]inode.DirEntry{
		{Name: "child", InodeID: "child-id", FileType: uint8(inode.ModeRegular >> 12)},
	}, nil)
	...
	inodeSvc.On("UnlinkBatch", mock.Anything, "dir-id", []string{"child"}).Return(nil)
	inodeSvc.On("UnlinkBatch", mock.Anything, "root-id", []string{"dir"}).Return(nil)
