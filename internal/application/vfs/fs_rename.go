...
func (s *service) Rename(ctx context.Context, cmd *RenameCommand) (*inode.Inode, error) {
	...
	entries, err := s.inodeOps.ReadDir(ctx, sourceParentDir.ID())
	if err != nil {
		return nil, err
	}
	...
	newEntry := inode.DirEntry{
		Name:     cmd.NewName,
		InodeID:  sourceInode.ID(),
		FileType: fileType,
	}
	if err := s.inodeOps.Link(ctx, sourceParentDir.ID(), newEntry); err != nil {
		return nil, err
	}

	if err := s.inodeOps.Unlink(ctx, sourceParentDir.ID(), srcEntryName); err != nil {
		return nil, err
	}
