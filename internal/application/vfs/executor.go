...
func (s *service) expandWildcards(ctx context.Context, systemID string, paths []string) ([]string, error) {
	...
	for _, p := range paths {
		...
		entries, err := s.inodeOps.ReadDir(ctx, dirInode.ID())
		if err != nil {
			return nil, err
		}
...
func (s *service) collectPath(ctx context.Context, systemID string, p string, recursive bool) ([]*deleteTarget, error) {
	...
	if len(childIDs) > 0 {
		childInodes, err = s.inodeOps.Find(ctx, inode.Filter{IDs: childIDs})
		if err != nil {
			return nil, err
		}
	}
...
func (s *service) collectDirectoryEntries(ctx context.Context, dirInode *inode.Inode) ([]inode.DirEntry, error) {
	entries, err := s.inodeOps.ReadDir(ctx, dirInode.ID())
	if err != nil {
		return nil, err
	}

	var result []inode.DirEntry
	for _, e := range entries {
		if e.Name == "." || e.Name == ".." {
			continue
		}
		...
		childInode, err := s.inodeOps.GetByID(ctx, e.InodeID)
		if err != nil {
			return nil, err
		}
		if childInode.IsDir() {
...
func (s *service) parallelDeleteInodes(ctx context.Context, targets []*deleteTarget) error {
	...
	for _, id := range inodeIDs {
		id := id // capture
		g.Go(func() error {
			if err := s.inodeOps.Delete(ctx, id); err != nil {
