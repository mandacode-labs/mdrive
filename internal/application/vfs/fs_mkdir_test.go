...
func TestMkdir_LinkFailureRollback(t *testing.T) {
	...
	// Link fails
	inodeSvc.EXPECT().Link(mock.Anything, "root-id", mock.Anything).Return(errors.Conflict("entry already exists"))
...
func TestMkdir_DotDotLinkFailureRollback(t *testing.T) {
	...
	inodeSvc.EXPECT().Link(mock.Anything, "new-dir-id", mock.MatchedBy(func(e inode.DirEntry) bool {
		return e.Name == ".."
	})).Return(errors.Internal("link failed"))

	// Rollback: unlink from parent and delete inode
	inodeSvc.EXPECT().Unlink(mock.Anything, "root-id", "testdir").Return(nil)
	inodeSvc.EXPECT().Delete(mock.Anything, "new-dir-id").Return(nil)
...
func TestMkdir_Success(t *testing.T) {
	...
	// Link into parent
	inodeSvc.EXPECT().Link(mock.Anything, "root-id", mock.MatchedBy(func(e inode.DirEntry) bool {
