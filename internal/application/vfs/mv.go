...
func (s *service) mvOne(ctx context.Context, systemID string, srcPath string, destPath string) error {
	...
	destEntries, err := s.inodeOps.ReadDir(ctx, destParentDir.ID())
	if err != nil {
		return err
	}
	...
	if err := s.inodeOps.Link(ctx, destParentDir.ID(), newEntry); err != nil {
		return err
	}

	return s.inodeOps.Unlink(ctx, sourceParentDir.ID(), srcEntryName)
}
