...
func TestBuildCredentials(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cfg:     Config{ClientID: "id", TokenIssuer: "x", Audience: "y", Scopes: "openid"},
			mode:    AuthModeClientCredentials,
			wantErr: true,
		...
		},
		{
			...
			cfg:     Config{ClientID: "id", ClientSecret: "s", Audience: "a", Scopes: "openid"},
			mode:    AuthModeClientCredentials,
			wantErr: true,
			errSub:  "requires token_issuer and audience",
		},
		{
			name:    "client_credentials_missing_scopes",
			cfg:     Config{ClientID: "id", ClientSecret: "s", TokenIssuer: "i", Audience: "a"},
			mode:    AuthModeClientCredentials,
			wantErr: true,
			errSub:  "requires scopes",
		},
		{
			name:    "client_credentials_with_api_token_rejected",
			cfg:     Config{APIToken: "x", ClientID: "y", ClientSecret: "z", TokenIssuer: "i", Audience: "a", Scopes: "openid"},
			mode:    AuthModeClientCredentials,
			wantErr: true,
