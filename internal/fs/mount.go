...
// sys_mount. ActionManageStorage on parent, ActionRead on
// source (so a non-viewable source cannot be mounted).
func (f *fs) BindMount(ctx context.Context, driveID, mountPath, sourceDriveID string) error {
	...
	if err := f.requireManageStorage(ctx, mountParent.DriveID); err != nil {
		return err
	}
	...
	if err := f.requireRead(ctx, srcULID); err != nil {
		return err
	}
