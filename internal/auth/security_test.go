...
func newSecurityTestService(t *testing.T, users user.Service) *Service {
	t.Helper()
	return &Service{
		encKey:       newTestKey(t),
...
func TestHandleCookieAuthRejectsInvalidCookie(t *testing.T) {
	svc := newSecurityTestService(t, newUserServiceMock(t, nil, errorx.New(errorx.KindNotFound, "")))
...
func TestHandleCookieAuthAttachesSessionOnSuccess(t *testing.T) {
	...
	svc := newSecurityTestService(t, newUserServiceMock(t, u, nil))
...
func TestHandleCookieAuthRejectsUnknownUser(t *testing.T) {
	users := newUserServiceMock(t, nil, errorx.New(errorx.KindNotFound, "user not found"))
...
// newUserServiceMock returns a ServiceMock whose GetByProviderID
...
func newUserServiceMock(t *testing.T, u *user.User, getErr error) *userMocks.ServiceMock {
	t.Helper()
