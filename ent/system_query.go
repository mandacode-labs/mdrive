...
func (_q *SystemQuery) QueryInodes() *InodeQuery {
...
func (_q *SystemQuery) WithInodes(opts ...func(*InodeQuery)) *SystemQuery {
	query := (&InodeClient{config: _q.config}).Query()
...
func (_q *SystemQuery) WithUsers(opts ...func(*UserQuery)) *SystemQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SystemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*System, error) {
	...
	if query := _q.withInodes; query != nil {
		if err := _q.loadInodes(ctx, query, nodes,
			func(n *System) { n.Edges.Inodes = []*Inode{} },
			func(n *System, e *Inode) { n.Edges.Inodes = append(n.Edges.Inodes, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withUsers; query != nil {
		if err := _q.loadUsers(ctx, query, nodes,
			...
			func(n *System, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SystemQuery) loadInodes(ctx context.Context, query *InodeQuery, nodes []*System, init func(*System), assign func(*System, *Inode)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(inode.FieldSystemID)
	}
...
func (_q *SystemQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*System, init func(*System), assign func(*System, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(system.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(system.UsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(system.UsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(system.UsersPrimaryKey[1]))
