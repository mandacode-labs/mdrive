...
func TestKind_Deployment(t *testing.T) {
	...
	t.Run("build and load image", func(t *testing.T) {
		...
		if cacheRef == "" {
			cacheRef = "registry.mandacode.com/mdrive/cache:main"
		}
		...
		cmd = exec.Command("kind", "load", "docker-image", "mdrive:test", "--name", suite.clusterName)
		output, err = cmd.CombinedOutput()
	...
	t.Run("install helm chart", func(t *testing.T) {
		suite.InstallHelmChart(t, "mdrive", "test")
	})

	// Verify deployment exists
	t.Run("deployment exists", func(t *testing.T) {
		suite.WaitForDeploymentExists(t, "mdrive")
	})

	// Verify pods were created
	t.Run("pods were created", func(t *testing.T) {
		podName := suite.GetPodName(t, "app.kubernetes.io/name=mdrive")
	...
	t.Run("migration job was created", func(t *testing.T) {
		require.Eventually(t, func() bool {
			return suite.JobExists(t, "mdrive-migration-1")
	...
	t.Run("pod reaches running state", func(t *testing.T) {
		require.Eventually(t, func() bool {
			return suite.PodIsRunning(t, "app.kubernetes.io/name=mdrive")
	...
	t.Run("service is accessible via port-forward", func(t *testing.T) {
		baseURL := suite.PortForward(t, "mdrive", 8080)
